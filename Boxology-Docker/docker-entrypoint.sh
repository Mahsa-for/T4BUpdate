...
#Import Shapes and Component for Boxology validation Plugin
  ...
  libraries: [
    {
      ...
      entries: [
		...
		{
          id: 'shape',
          title: { main: 'Shape Library' }, 
          preview: window.location.origin + '/images/sidebar-shape.png',
          libs: [
			{
              title: { main: 'Shapes' }, 
              url: window.location.origin + '/lib/ShapeLib.xml',
              noProxy: true,
              prefetch: true
            }
		...
		{
          title: { main: 'Pattern Library' },
          id: 'pattern',
          preview: window.location.origin + '/images/sidebar-pattern.png',
          libs: [
            {
              title: { main: 'Patterns' }, 
              url: window.location.origin + '/lib/PatternLib.xml',
              noProxy: true,
              prefetch: true
            }
        ...
        },
		{
          ...
          preview: window.location.origin + '/images/sidebar-annotation.png',
          libs: [
			{
...
#Import Boxology Validation Plugin

cat <<'EOF' >> $CATALINA_HOME/webapps/draw/js/PreConfig.js

(function waitForDrawPlugin() {
  if (typeof Draw !== 'undefined' && typeof Draw.loadPlugin === 'function') {
    Draw.loadPlugin(function(editorUi) {
      // Load Plugin
      var pluginScript = document.createElement('script');
      pluginScript.src = '/plugins/BoxologyValidation.js';
      pluginScript.onload = () => console.log("✅ Plugin loaded");
      pluginScript.onerror = () => console.error("❌ Failed to load plugin");
      document.head.appendChild(pluginScript);
    });
  } else {
    console.log("⌛ Waiting for Draw to be defined...");
    setTimeout(waitForDrawPlugin, 100);
  }
})();
EOF
